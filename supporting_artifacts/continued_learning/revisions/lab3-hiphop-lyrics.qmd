...
The dataset is comparing students' musical tastes for hip-hop to how familiar with African-American English vocabulary. The study was conducted at the University of Minnesota, on undergraduate students in linguistics, sociology, and music classes. The study was undertaken in order to figure out if there was a connection between hip-hop preference and knowledge of the AEE slang. ::: callout-note It is always good practice to start an analysis by getting a feel for the data and providing a quick summary for readers. You do not need to show any source code for this question, although you probably want to use code to get some information about the data set. :::

**3. What are the rows of this data set?** The rows of this dataset represent a word of slang tested per student participant. Since there 168 people, and there are 64 words, so if each person got tested for each of the 64 words, 168\*64 = 10752 total rows. ::: callout-warning It is not one person per row! :::
...
Missing values were replaced with mean values of the -Move variables and blackWeekly. Some benefits of this would be that it is easier to fill in these values, since they are more representative of the sample size, and it isn't an arbitrary value / NA. This means that you can perform counts and other operations on rows and columns with each datapoint being of numeric value. Some drawbacks are that the new datapoints may not be closely accurate to what the actual participants believe, and it might skew the data towards the mean if there are a lot of missing values present.
...
There are 64 unique AAE words studied in this dataset.
...
hiphop_clean <- hiphop_clean |>
  mutate(hiphop_clean, ethnic = if_else(
    ethnic == 'white', "white", 'non-white'))
hiphop_clean

```

**8. It is fairly common for researchers to collapse ethnic or racial categories similar to what you just did. What are some issues with representing the data in this way?** Some issues with that is that the audience does not know which racial or ethnic group is being represented, and therefore is ambiguous to the results. Some groups might feel discriminated against if this was the way that the data was presented, because their group is forced into this
...
According to the above analysis, the mean age is around 19 years old, with ages spanning between 16 and 42. There are more females than males (3584 vs. 1536), respectively. There are more white than non-white participants (3968 vs 1152).
...
ggplot(data=hiphop_clean) + 
  geom_histogram(mapping = aes(x= ethnic, fill = sex), 
  position = position_dodge()) +
  labs(x ="Ethnicity", title = "Subject Ethnicity by Sex")

ggplot(data=hiphop_clean) + 
  geom_histogram(mapping = aes(x = age, fill = sex),
  position = position_dodge()) +
  labs(x = 'Age', title = 'Subject Age by Sex')
```
...
The words that were most familiar with those under 20 were feel me, off the hook, etc. and the words that were least familiar were The Nation, catch the vapors, etc. on average.
...
# code chunk for Q12
hiphop_clean |>
  filter(ethnic == 'non-white', sex == "Female", familiarity == 5) |>
  arrange(familiarity, desc(familiarity), by_group = TRUE) |>
  group_by(familiarity) |>
  count(word, sort=TRUE)

hiphop_clean |>
  filter(ethnic == 'non-white', sex == "Female", familiarity == 1) |>
  arrange(familiarity, desc(familiarity), by_group = TRUE) |>
  group_by(familiarity) |>
  count(word, sort=TRUE)
```

The words that were most familiar among non-white women were feel me, what it do, etc. and the words that were least familiar among non-white women were break someone out, dollar cab, etc.
...
The words that were most familiar among white men above age 30 were 5-0, hard, etc. and the least familiar were \[to be\] ghost, A-town, etc. \## Study Subjects
...
# code chunk for Q14
hiphop_clean |>
  filter(bieber == 5, ethnic == "white", sex == "Male", age > 17, age < 23)
slice_max(bieber)
```

Based on the prior analysis, it looks like subj p17 is Justin Bieber, since he is the only one who scored highest on who could name up to 5 JB songs. He is also between 17 and 23, and a white male, so it makes sense that this could be him.
