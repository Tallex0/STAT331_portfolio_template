...
*Hint: Writing code is Step 1. You also need to connect the output of the code to what it tells you about the dimensions of the data.* The dimensions are 30,463 x 15. RowsxCols. \### **Question 3**

What are the data types of the variables in the dataset?\
*Hint: Yes, this question requires code. No, you do not need anything complicated.* The data types are double, character, and date. \## Plotting with **`ggplot2`**
...
ggplot(data=surveys, mapping = aes(x=weight, y=species)) +
  ...
  labs(x="Species", y="Weight", title = "Animal Species vs. Weight")
...
ggplot(data=surveys, mapping = aes(x=weight, y=species)) +
  ...
  labs(x="Weight (g)", y="Species", title = "Animal Species vs. Weight")
```
